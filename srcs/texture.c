...
void		load_texture3(t_env *e)
{
	...
	&e->naz, &e->end);
	if (!((e->text_img[13] = mlx_xpm_file_to_image(e->mlx.mlx_ptr,
	"./textures/sun.xpm", &(e->sl[13]), &e->bpp[13]))))
		ft_kill("Texture error");
	e->text_data[13] = (unsigned char*)mlx_get_data_addr(e->text_img[13], &e->naz,
	&e->naz, &e->end);
}
