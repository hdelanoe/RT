...
int		check_keycode(int keycode, t_env *e)
{
	...
	if (keycode == KEY_D || keycode == KEY_A || keycode == KEY_Q ||
		...
		keycode == KEY_H || keycode == KEY_PAD_ADD || keycode == KEY_PAD_7 ||
		keycode == KEY_PAD_EQUAL || keycode == KEY_PAD_DIVIDE ||
		keycode == KEY_PAD_MULTIPLY ||
		keycode == KEY_PAD_SUB || keycode == KEY_I ||
		keycode == KEY_C || keycode == KEY_F || keycode == KEY_Y ||
		keycode == KEY_M || keycode == KEY_O || KC == KEY_L || KC == KEY_J ||
		keycode == KEY_U || keycode == KEY_K)
		return (1);
	return (0);
}
