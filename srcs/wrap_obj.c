...
/*   Updated: 2018/03/06 15:14:34 by hdelanoe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_color	wrap_plane(t_env *e, t_object *object)
{
	...
	{
		x = fabs(object->point.z - object->point_2.z);
		xx = fabs(object->point.z - object->node.z);
	}
	if (fabs(object->node_normal.z) < fabs(object->node_normal.y))
	{
		y = fabs(object->point.z - object->point_3.z);
